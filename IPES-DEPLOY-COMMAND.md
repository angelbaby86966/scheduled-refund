...
>
> 脚本：`ipes_deploy_full.sh`（**v2026-09-13-r17**，SHA `0ce5d0cd`）
> r15 提速：nload 改后台并行安装；修复业务ID/属性校验的解析 bug（`sed '$d'` 误删单行 JSON，导致每台白等 ~216s）；轮询 180s→30s。
> **r16 裸机加固**：清掉 sysconfig docker flag（`--log-driver/--storage-driver`）与 ecache 写入 daemon.json 的冲突 + ecache 后 docker 自愈（实测杭州裸机 CentOS-rtms 踩坑：docker 起不来 → 容器丢失 → 业务ID 占位符）。
> **单机全流程实测 ~1 分钟**（容器已存在的克隆机）。
> 主源（jsDelivr，锁定 commit）：
> `https://cdn.jsdelivr.net/gh/angelbaby86966/scheduled-refund@0ce5d0cd54710c669daac3779dce54cd64eb42e8/ipes_deploy_full.sh`
> 备源（ghproxy，带时间戳穿透缓存）：
> `https://ghproxy.net/https://raw.githubusercontent.com/angelbaby86966/scheduled-refund/main/ipes_deploy_full.sh?t=$(date +%s)`
>
> **🚀 启动器 v2（2026-09-13 22:15）**：双源**并行**下载（各限 15s，最坏 ~17s 出结果，替代旧的串行 60s 超时）+ 双重校验（`singleIpRadio` 内容 + `v2026-09-13-r17` 精确版本号），两源都拿不到时**拒绝启动**（报"下载失败"），杜绝跑旧版。
