...
(function() {
  ...
  window.AliyunClient = {
    ...
    /**
     * 创建配额提升申请（QuotaCenter CreateQuotaApplication）
     * 浏览器无法直连 quotacenter.aliyuncs.com，必须走 aliyun-proxy Edge Function
     */
    async createQuotaApplication(regionId, desireValue, opts) {
      opts = opts || {};
      var ak = getAccessKeyId(), sk = getAccessKeySecret();
      if (!ak || !sk) throw new Error('请先设置阿里云 AK/SK 凭证');
      var resp = await fetch(window.WB_SUPABASE_FUNCTIONS + '/aliyun-proxy', {
        ...
        body: JSON.stringify({
          ...
          params: {
            RegionId: regionId,
            ProductCode: opts.productCode || 'swas',
            QuotaActionCode: opts.quotaActionCode || 'q_z3sbl5',
            DesireValue: desireValue || 500,
            Reason: opts.reason || '业务扩展，需提升实例数量上限',
            NoticeType: opts.noticeType != null ? opts.noticeType : 0,
            QuotaCategory: opts.quotaCategory || 'CommonQuota'
          }
        })
      });
      var j = await resp.json();
      if (!j.success) {
        var e = new Error(j.error || '创建配额申请失败');
        e.response = j;
        throw e;
      }
      return j.data;
