...
// 【2026-09-19 调整】目标上限固定为 500 台（按需求撤掉「可输入 501~1000」的输入框）。
//   要改数值，只改下面这一个常量即可。
//   注意：配额 q_z3sbl5 的 ApplicableRange 是 [501, 1000] 且申请值必须高于当前值，
//   所以对「当前已是 500」的地区提交 500 会被阿里云拒绝
//   （The applied quota value is invalid.）—— 属预期行为，直接跳过即可。
var QUOTA_DESIRE_VALUE = 500;

function batchApplyQuotaFixed() {
  return batchApplyQuota(QUOTA_DESIRE_VALUE);
}

// 兼容旧页面（可能缓存着带输入框的老 index.html）：老按钮名仍可用，一律按固定值提交。
function batchApplyQuotaFromInput() {
  return batchApplyQuotaFixed();
}
    ...
    try {
      var resp = await fetch(OCD_SUPABASE_FN, {
        ...
        body: JSON.stringify({
          ...
          path: (function(p){return String(p||'').replace(/^(https?:\/\/[^\/]+)?\/backend\/api\//,function(m,h){return (h||'')+'/api/';});})(adv.path || '/api/edgeNode/getEdgeNodeList'),
          query: query,
          body: null
        })
      });
