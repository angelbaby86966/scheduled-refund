...
#     --jwt eyJ... \
#     --isp 移动 [--province 浙江 --city 杭州 --num-dirs 12 --usbw 200]
# 未传 --province/--city 时，自动按本机公网 IP 识别；识别失败兜底为 浙江/杭州。
set +e
# [REV] wrapper-finish-passthrough-20260919b-chancache

AK=""; SK=""; JWT=""; ISP="移动"; PROVINCE=""; CITY=""; NUM_DIRS=12; USBW=200; BW_NUM=1
...
if [ "${FINISH_ONLY:-0}" != "1" ] && [[ -z "$AK" || -z "$SK" || -z "$JWT" ]]; then
  ...
  echo "示例：curl -fsSL ... | bash -s -- --ak <ak> --sk <sk> --jwt <jwt> --isp 移动"
  exit 1
fi
