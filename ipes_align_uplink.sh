...
# 【用法】
#   # 正式执行（改完自动复测）
#   curl -fsSL https://ghproxy.net/https://raw.githubusercontent.com/angelbaby86966/ipes-scripts/main/ipes_align_uplink.sh | bash
#
#   # 只体检不改动（先看差在哪，再决定改不改）
#   curl -fsSL https://ghproxy.net/https://raw.githubusercontent.com/angelbaby86966/ipes-scripts/main/ipes_align_uplink.sh | bash -s -- --check
#
#   # 可选环境变量
#   #   TARGET_HAPP=9       happ 实例数对齐目标（默认 9，当前 >= 目标则跳过）
#   #   TARGET_TAG=1.3.0    IPES 镜像 tag 对齐目标（默认 1.3.0）
#   #   ALIGN_HAPP=1        确认补 happ 并重写 docker_run（默认 0=只报告，不冒险改）
...
align_ipes(){
  ...
  local C=ipes DR=/opt/ipes/docker_run
  ...
  # (a) happ 结构：默认只报告。happ 实例数最终由平台按本机 SN 下发决定，
  #     且改写 docker_run 风险高，因此仅当 ALIGN_HAPP=1 时按「已验证的标准模板」重写。
  if [ "$cur_happ" -lt "$TARGET_HAPP" ] 2>/dev/null; then
    ...
    if [ "$ALIGN_HAPP" = "1" ]; then
      ...
      for i in $(seq 0 $((TARGET_HAPP-1))); do
        ...
        if [ -d "$xbi" ]; then
          warn "$xbi 是目录（上次失败遗留），会导致 IPES 起不来；请人工确认后再重跑"
        elif [ ! -e "$xbi" ]; then
