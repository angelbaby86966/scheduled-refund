...
# [6.7] IPES 应用层保活（每分钟）：容器级 docker start 兜底 + 【r20-fix11 挂载硬自愈】+ health 探测失败恢复。
#   与看门狗分工：watchdog → 进程层（master 拉起 + happ 防回弹）；本模块 → 容器/服务层。
#   融合来源：渠道 install_ipes_health_check.sh；未照抄其伤机器三处（crontab 误删全部含 ipes 的 cron、
#   read 交互提问、@reboot 写 `bin/ipes stop` 重启后停业务）。
install_ipes_health() {
    print_step "安装 IPES 保活（每分钟：容器兜底启动 + fix11 挂载自愈 + health 探测）"
...
# 【r20-fix】对齐 happ worker 数到 $TARGET_HAPP（2026-09-25 起语义=初始下限 9：初始 9 路，平台推到 12 不干预，低于 9 看门狗自动补回）
...
main() {
    ...
    # [6.7] 安装应用层保活：每分钟容器兜底启动 + fix11 挂载自愈 + health 探测失败恢复
    install_ipes_health
