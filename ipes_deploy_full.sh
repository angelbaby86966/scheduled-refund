...
# =============================================================================
# 【r20-fix14/fix15】绑定成功后：先提交业务41(q2)+200M，再流转「服务中」；业务ID 后置
# -----------------------------------------------------------------------------
# 背景（用户原话）："绑定后，先从待配置，转到服务中，之后再慢慢走流程，部署，填业务id"
# 动机：注册完成后还要等拉镜像 / 磁盘调优 / 预热好几轮，期间后台一直停在「待配置」，
#       运维会误以为绑定失败。这里在 [4] 注册成功后**立刻**把状态顶到「服务中」，
#       让后台马上脱离「待配置」。
# 【r20-fix15】用户追加要求："转服务中之前，改成41业务q2，改成200m，再流转服务中"。
#       平台规则（实测）：只有「待配置」状态才允许改设备信息（服务中/交付中会返回 code:7），
#       故本函数内顺序固定为：确保待配置(downgrade_to_configured) → 提交业务 ${BUSINESS_ID}(q2)
#       + 带宽 ${NODE_USBW}M(submit_business) → 流转「服务中」。复用既有函数，幂等。
...
#       避免把后台已写入的真业务ID覆盖成占位符；业务/带宽提交同样幂等，与 [9] 重复执行无害。
...
early_transition_to_serving() {
    ...
    # 【r20-fix15】用户要求："转服务中之前，改成41业务q2，改成200m，再流转服务中"。
    #   平台规则：只有「待配置」状态才允许改设备信息（在服务中/交付中会返回 code:7），
    #   故顺序必须是：确保待配置 → 提交业务 ${BUSINESS_ID}(q2) + 带宽 ${NODE_USBW}M → 流转「服务中」。
    #   复用既有函数（幂等）：与后面的 [9] 重复执行无害；[9] 会在容器起来后带真 76hex 业务ID 再覆盖一次。
...
# 【r20-fix11 运行时硬自愈】容器 docker start 失败时，先修 happ 挂载类型
#   （xycould_base_info 被补成目录 → 43B 文件）再重试，根治"容器 Exited 127 → 永久零上下行"。
#   部署期已有 ensure_happ_base_info_files；此处补运行期，防止部署后挂载类型被再次破坏。
